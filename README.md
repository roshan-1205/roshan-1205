<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:2E3192,50:1BFFFF,100:D4145A&height=200&section=header&text=Roshan%20Kumar%20Singh&fontSize=50&fontAlignY=35&desc=B.Tech%20CSE%20|%20Full%20Stack%20Developer%20|%20AI%20Engineer&descAlignY=55&descAlign=50&animation=fadeIn&fontColor=fff" width="100%"/>
</div>

<div align="center">
  
[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=00D9FF&center=true&vCenter=true&width=900&height=80&lines=Building+Intelligent+Solutions+with+AI+%26+Cloud+%E2%98%81%EF%B8%8F;Transforming+Ideas+into+Scalable+Applications+%F0%9F%9A%80;Passionate+About+Civic+Tech+%26+Social+Impact+%F0%9F%8C%8D)](https://git.io/typing-svg)

</div>

<div align="center">
  
  ![Profile Views](https://komarev.com/ghpvc/?username=roshan-1205&color=2E3192&style=for-the-badge&label=PROFILE+VIEWS)
  [![GitHub followers](https://img.shields.io/github/followers/roshan-1205?label=Followers&style=for-the-badge&color=1BFFFF)](https://github.com/roshan-1205)
  [![GitHub Stars](https://img.shields.io/github/stars/roshan-1205?label=Stars&style=for-the-badge&color=D4145A)](https://github.com/roshan-1205)
  
</div>

---

## 👨‍💻 About Me

<img align="right" alt="Coding" width="400" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif">

```javascript
const roshan = {
    location: "India 🇮🇳",
    education: "B.Tech in Computer Science",
    role: "Full Stack Developer & AI Engineer",
    currentFocus: ["AI Engineering", "Cloud Architecture", "System Design"],
    workingOn: ["Voice AI Applications", "Civic Tech Solutions"],
    learning: ["Advanced AWS Services", "RAG Systems", "GATE Preparation"],
    techStack: {
        frontend: ["React.js", "JavaScript ES6", "HTML5", "CSS3"],
        backend: ["Node.js", "Python", "FastAPI"],
        cloud: ["AWS Lambda", "API Gateway", "S3", "DynamoDB"],
        ai: ["OpenAI APIs", "NLP", "Voice AI", "RAG Systems"]
    },
    goal: "Building AI-powered solutions that create real-world impact",
    philosophy: "Technology should bridge gaps and empower communities"
};
```

<br/>

I'm a **B.Tech Computer Science student** and passionate **Full Stack Developer** specializing in building intelligent, scalable applications that solve real-world problems. My expertise spans modern web technologies, cloud infrastructure, and artificial intelligence.

🎯 **What I Do:**
- 🚀 Build full-stack applications using MERN stack
- 🤖 Develop AI-powered solutions with OpenAI APIs and NLP
- ☁️ Deploy scalable applications on AWS cloud infrastructure
- 🎙️ Create voice-first applications for accessibility

🌱 **Currently Learning:** AI Engineering, System Design, Cloud Architecture, and preparing for GATE

💡 **Mission:** Creating technology solutions that drive social impact and empower communities

---

## 🛠️ Tech Stack & Skills

<div align="center">

### 💻 Frontend Development
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript_ES6-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React.js-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Responsive Design](https://img.shields.io/badge/Responsive_Design-FF6B6B?style=for-the-badge&logo=css3&logoColor=white)

### ⚙️ Backend Development
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white)

### 🗄️ Databases
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![DynamoDB](https://img.shields.io/badge/Amazon_DynamoDB-4053D6?style=for-the-badge&logo=amazondynamodb&logoColor=white)

### ☁️ Cloud & DevOps
![AWS](https://img.shields.io/badge/Amazon_AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![Lambda](https://img.shields.io/badge/AWS_Lambda-FF9900?style=for-the-badge&logo=awslambda&logoColor=white)
![API Gateway](https://img.shields.io/badge/API_Gateway-FF4F8B?style=for-the-badge&logo=amazonapigateway&logoColor=white)
![S3](https://img.shields.io/badge/Amazon_S3-569A31?style=for-the-badge&logo=amazons3&logoColor=white)
![Cognito](https://img.shields.io/badge/AWS_Cognito-DD344C?style=for-the-badge&logo=amazonaws&logoColor=white)
![Amplify](https://img.shields.io/badge/AWS_Amplify-FF9900?style=for-the-badge&logo=awsamplify&logoColor=white)
![CloudFront](https://img.shields.io/badge/CloudFront-8C4FFF?style=for-the-badge&logo=amazonaws&logoColor=white)

### 🤖 AI & Machine Learning
![OpenAI](https://img.shields.io/badge/OpenAI_API-412991?style=for-the-badge&logo=openai&logoColor=white)
![NLP](https://img.shields.io/badge/NLP-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![Voice AI](https://img.shields.io/badge/Voice_AI-4285F4?style=for-the-badge&logo=googleassistant&logoColor=white)
![RAG Systems](https://img.shields.io/badge/RAG_Systems-00ADD8?style=for-the-badge&logo=ai&logoColor=white)

### 🛠️ Tools & IDEs
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![npm](https://img.shields.io/badge/npm-CB3837?style=for-the-badge&logo=npm&logoColor=white)

</div>

---

## ☁️ Cloud & AI Stack

<div align="center">

### AWS Services
| Service | Usage |
|---------|-------|
| **Lambda** | Serverless compute for backend functions |
| **API Gateway** | RESTful API management |
| **S3** | Static hosting & file storage |
| **DynamoDB** | NoSQL database for scalable apps |
| **Cognito** | User authentication & authorization |
| **Amplify** | Full-stack deployment |
| **CloudFront** | CDN for global content delivery |

### AI & ML Tools
| Technology | Application |
|------------|-------------|
| **OpenAI APIs** | GPT integration for intelligent features |
| **FastAPI** | High-performance API backend |
| **NLP Models** | Natural language processing |
| **Voice AI** | Speech recognition & synthesis |
| **RAG Pipelines** | Retrieval-augmented generation |
| **Vector Databases** | Semantic search & embeddings |

</div>

---

## 🚀 Featured Projects

<div align="center">

### 🎙️ VAANI - Voice-First AI Civic Platform

[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-181717?style=for-the-badge&logo=github)](https://github.com/roshan-1205/VAANI)
![Status](https://img.shields.io/badge/Status-Active-success?style=for-the-badge)
![Stars](https://img.shields.io/github/stars/roshan-1205/VAANI?style=for-the-badge&color=yellow)

</div>

**A revolutionary voice-first platform empowering citizens to report civic issues using AI technology**

#### 🎯 Project Overview
VAANI is an innovative civic engagement platform that leverages voice AI to make civic participation accessible to everyone, including those with limited literacy or physical disabilities. The platform enables citizens to report civic issues, track their status, and receive real-time updates through natural voice interactions.

#### ✨ Key Features
- 🗣️ **Voice Assistant** - Hands-free interaction using advanced speech recognition
- 📍 **Civic Issue Reporting** - Report problems with geolocation tagging
- 🔔 **Real-time Alerts** - Instant notifications on issue status updates
- 🔐 **JWT Authentication** - Secure user authentication and authorization
- 🧠 **NLP Layer** - Intelligent query processing and intent recognition
- 📊 **Dashboard** - Track and manage reported issues
- 🌐 **Multi-language Support** - Accessible to diverse user base

#### 🛠️ Technology Stack
```
Frontend:  React.js, HTML5, CSS3, JavaScript ES6
Backend:   FastAPI, Python
AI/ML:     OpenAI APIs, NLP Models, Voice AI
Database:  MongoDB
Cloud:     AWS (Lambda, S3, API Gateway)
Auth:      JWT Authentication
```

#### 🔗 Links
- **Repository:** [github.com/roshan-1205/VAANI](https://github.com/roshan-1205/VAANI)
- **Live Demo:** Coming Soon

---

<div align="center">

### 💜 HamariBeti - Women Empowerment Platform

![Status](https://img.shields.io/badge/Status-Active-success?style=for-the-badge)
![Impact](https://img.shields.io/badge/Impact-Social-ff69b4?style=for-the-badge)

</div>

**A comprehensive digital platform dedicated to women's safety, empowerment, and community support**

#### 🎯 Project Overview
HamariBeti is a women-centric platform designed to provide safety features, community support, and empowerment resources. The platform integrates government verification systems and emergency response features to create a secure ecosystem for women.

#### ✨ Key Features
- 🆔 **Aadhaar Integration** - Secure OTP-based verification system
- 🔒 **Authentication System** - Multi-layer security with JWT
- 🆘 **SOS Emergency Support** - One-tap emergency alert system
- 📱 **Real-time Alerts** - Instant notifications to emergency contacts
- 👥 **Community Support** - Connect with support groups and resources
- 📚 **Resource Hub** - Access to empowerment programs and schemes
- 🛡️ **Privacy First** - End-to-end encrypted communications

#### 🛠️ Technology Stack
```
Frontend:  React.js, JavaScript ES6, Responsive Design
Backend:   Node.js, Express.js
Database:  MongoDB
APIs:      Aadhaar API, SMS Gateway
Auth:      JWT, OAuth 2.0
Security:  Encryption, Secure Storage
```

#### 🔗 Links
- **Repository:** Coming Soon
- **Live Demo:** Coming Soon

---

## 📊 GitHub Analytics

<div align="center">
  
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=roshan-1205&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=0D1117&title_color=58A6FF&icon_color=1F6FEB&text_color=C9D1D9&ring_color=1F6FEB"/>
  
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=roshan-1205&layout=compact&langs_count=8&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&text_color=C9D1D9"/>

</div>

<div align="center">
  
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=roshan-1205&theme=tokyonight&hide_border=true&background=0D1117&stroke=58A6FF&ring=1F6FEB&fire=FF6B6B&currStreakLabel=C9D1D9" />

</div>

<div align="center">
  
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=roshan-1205&theme=tokyo-night&hide_border=true&bg_color=0D1117&color=58A6FF&line=1F6FEB&point=C9D1D9" width="100%"/>

</div>

---

## 🐍 Contribution Activity

<div align="center">
  
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/roshan-1205/roshan-1205/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/roshan-1205/roshan-1205/output/github-contribution-grid-snake.svg">
    <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/roshan-1205/roshan-1205/output/github-contribution-grid-snake.svg">
  </picture>

</div>

---

## 📚 Currently Learning

<div align="center">

<table>
<tr>
<td align="center" width="50%">

### 🤖 AI Engineering
![RAG Systems](https://img.shields.io/badge/RAG_Systems-00D9FF?style=for-the-badge&logoColor=white)
![Vector DB](https://img.shields.io/badge/Vector_Databases-FF6B6B?style=for-the-badge&logoColor=white)
![LLM](https://img.shields.io/badge/LLM_Fine--tuning-9D4EDD?style=for-the-badge&logoColor=white)

</td>
<td align="center" width="50%">

### 🏗️ System Design
![Scalability](https://img.shields.io/badge/Scalability-00F5FF?style=for-the-badge&logoColor=white)
![Microservices](https://img.shields.io/badge/Microservices-FF006E?style=for-the-badge&logoColor=white)
![Load Balancing](https://img.shields.io/badge/Load_Balancing-8338EC?style=for-the-badge&logoColor=white)

</td>
</tr>
<tr>
<td align="center" width="50%">

### ☁️ Cloud Architecture
![AWS Advanced](https://img.shields.io/badge/AWS_Advanced-FFD60A?style=for-the-badge&logoColor=black)
![Serverless](https://img.shields.io/badge/Serverless_Patterns-06FFA5?style=for-the-badge&logoColor=black)
![IaC](https://img.shields.io/badge/Infrastructure_as_Code-FF5400?style=for-the-badge&logoColor=white)

</td>
<td align="center" width="50%">

### 📖 GATE Preparation
![DSA](https://img.shields.io/badge/Data_Structures-00D9FF?style=for-the-badge&logoColor=white)
![Algorithms](https://img.shields.io/badge/Algorithms-FF006E?style=for-the-badge&logoColor=white)
![Networks](https://img.shields.io/badge/Computer_Networks-9D4EDD?style=for-the-badge&logoColor=white)

</td>
</tr>
</table>

</div>

<div align="center">

**🎯 Focus Areas:**

🤖 Building production-ready AI applications with RAG systems and vector databases

🏗️ Architecting scalable distributed systems with microservices patterns

☁️ Mastering AWS advanced services and serverless architecture

📖 Strengthening computer science fundamentals for GATE

</div>

---

## 🏆 Achievements & Milestones

<div align="center">

| 🎯 Achievement | 📅 Status |
|---------------|----------|
| Built 2+ Production-Ready Projects | ✅ Completed |
| Integrated Voice AI in Applications | ✅ Completed |
| Deployed Apps on AWS Cloud | ✅ Completed |
| Mastered MERN Stack | ✅ Completed |
| Learning AI Engineering | 🔄 In Progress |
| GATE Preparation | 🔄 In Progress |

</div>

### 🎓 Certifications & Hackathons
- 🏅 Participated in multiple hackathons
- 💻 Built civic tech and social impact projects
- 🚀 Deployed scalable cloud applications

---

## 🤝 Connect With Me

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-roshan--1205-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/roshan-1205)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Roshan_Kumar_Singh-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/roshan-kumar-singh-9b9191299)
[![Email](https://img.shields.io/badge/Email-roshankumar021@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:roshankumar021@gmail.com)
[![LeetCode](https://img.shields.io/badge/LeetCode-roshan--1205-FFA116?style=for-the-badge&logo=leetcode&logoColor=white)](https://leetcode.com/roshan-1205)

</div>

<div align="center">

### 📧 Get In Touch

I'm always open to discussing new projects, creative ideas, or opportunities to be part of your vision.

**Email:** roshankumar021@gmail.com

**LinkedIn:** [Roshan Kumar Singh](https://www.linkedin.com/in/roshan-kumar-singh-9b9191299)

</div>

---

<div align="center">

### 💭 Developer Quote

![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight)

</div>

---

<div align="center">

### 💡 *"The best way to predict the future is to invent it."* – Alan Kay

### ⭐ From [Roshan Kumar Singh](https://github.com/roshan-1205)

**If you find my work interesting, feel free to star ⭐ my repositories!**

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2E3192,50:1BFFFF,100:D4145A&height=120&section=footer" width="100%"/>

</div>


---

## 🐍 Setup Snake Animation (GitHub Action)

To enable the contribution snake animation on your profile, follow these steps:

### Step 1: Create Workflow File

Create a file at `.github/workflows/snake.yml` in your profile repository with this content:

```yaml
name: Generate Snake Animation

on:
  schedule:
    - cron: "0 */24 * * *"
  workflow_dispatch:
  push:
    branches:
      - main

jobs:
  generate:
    permissions:
      contents: write
    runs-on: ubuntu-latest
    timeout-minutes: 5
    
    steps:
      - name: Checkout repository
        uses: actions/checkout@v3
      
      - name: Generate github-contribution-grid-snake.svg
        uses: Platane/snk/svg-only@v3
        with:
          github_user_name: roshan-1205
          outputs: |
            dist/github-contribution-grid-snake.svg
            dist/github-contribution-grid-snake-dark.svg?palette=github-dark
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
      
      - name: Push github-contribution-grid-snake.svg to output branch
        uses: crazy-max/ghaction-github-pages@v3.1.0
        with:
          target_branch: output
          build_dir: dist
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```

### Step 2: Enable GitHub Actions

1. Go to your repository **Settings**
2. Navigate to **Actions** → **General**
3. Under **Workflow permissions**, select **Read and write permissions**
4. Click **Save**

### Step 3: Run the Workflow

1. Go to **Actions** tab in your repository
2. Click on **Generate Snake Animation**
3. Click **Run workflow** → **Run workflow**
4. Wait for completion (1-2 minutes)

The snake animation will now appear in your README! 🎉
